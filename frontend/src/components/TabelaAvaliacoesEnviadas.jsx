...
export const TabelaAvaliacoesEnviadas = ({ avaliacoes }) => {
  ...
  const avaliacoesFiltradas = useMemo(() => {
    return avaliacoes.filter((avaliacao) => {
      const pesquisaValida = avaliacao.nome
        .toLowerCase()
        .includes(pesquisa.toLowerCase());

      const tipoValido = tipo === "Todos" || avaliacao.tipo === tipo;

      const statusValido = status === "Todos" || avaliacao.status === status;

      return pesquisaValida && tipoValido && statusValido;
    });
        ...
        <table className="w-full">
          <thead>
            <tr className="text-gray-500 text-sm border-b">
              <th className="pb-4 text-left">AVALIAÇÃO</th>

              <th className="pb-4 text-center">TIPO</th>

              <th className="pb-4 text-center">TURMA</th>

              <th className="pb-4 text-center">POSTAGEM</th>
          ...
          <tbody>
            {avaliacoesFiltradas.map((avaliacao) => (
              <LinhaAvaliacaoEnviada key={avaliacao.id} avaliacao={avaliacao} />
            ))}
          </tbody>
        </table>
      </div>
