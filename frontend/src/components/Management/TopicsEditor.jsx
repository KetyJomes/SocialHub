...
export const TopicsEditor = ({ secoes, setSecoes, adicionarSecao }) => {
  ...
  return (
    <section className="mt-8 space-y-6">
      {secoes.map((secao) => (
        ...
        >
          <h2 className="text-xl font-bold text-gray-800 mb-5">Tópico</h2>
          ...
          <div className="mt-6 space-y-4">
            {secao.perguntas.map((pergunta, index) => (
              <EvaluationQuestionCard
                ...
                pergunta={pergunta}
                onChange={(valor) =>
                  alterarPergunta(secao.id, pergunta.id, valor)
                }
                onChangeEscala={(escala) =>
                  alterarEscala(secao.id, pergunta.id, escala)
                }
                escalasDisponiveis={[
                  "Crítico",
                  "Abaixo do esperado",
