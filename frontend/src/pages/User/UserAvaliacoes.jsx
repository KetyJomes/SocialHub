...
export const UserAvaliacoes = () => {
  ...
  useEffect(() => {
    ...
    setAvaliacoes((prev) =>
      prev.map((avaliacao) => {
        const avaliacaoSalva = avaliacoesSalvas[avaliacao.id];
  ...
  return (
    <>
      ...
      <main className="mt-[8vh] min-h-[calc(100vh-11.5vh)]">
        <div className="p-10">
          <h1 className="font-bold text-3xl mb-2">Avaliações</h1>
          ...
          <section className="mt-8">
            {abaAtiva === "disponiveis" && (
              <AbaAvaliacoes avaliacoes={disponiveis} />
            )}

            {abaAtiva === "feitas" && (
              <AbaAvaliacoes avaliacoes={feitas} modoVisualizacao={true} />
            )}
          </section>
        </div>
      </main>
