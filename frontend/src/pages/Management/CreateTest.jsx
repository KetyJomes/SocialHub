...
export const CreateTest = () => {
  ...
  const [turma, setTurma] = useState(modelo?.turma || "");
  ...
  useEffect(() => {
    ...
    setTipoPublico(modelo.tipoPublico || "todos");

    setAlunosSelecionados(modelo.alunosSelecionados || []);
  ...
  function criarAvaliacao() {
    ...
    if (tipoPublico === "alguns" && alunosSelecionados.length === 0) {
      setMensagemAlert(
    ...
    if (secoes.length < 1) {
      setMensagemAlert("Crie pelo menos um tópico para a avaliação.");
    ...
    for (const secao of secoes) {
      if (!secao.titulo.trim()) {
        setMensagemAlert("Todos os tópicos devem possuir um título.");
      ...
      const possuiTodasEscalas = escalasObrigatorias.every((escala) =>
        escalasUtilizadas.includes(escala)
      );
      ...
      for (const pergunta of secao.perguntas) {
        if (!pergunta.texto.trim() || !pergunta.escala) {
          setMensagemAlert(
  ...
  function salvarModelo() {
    ...
    const index = modelsMock.findIndex((item) => item.id === modeloOrigemId);
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <section className="mt-8 bg-white border border-gray-200 rounded-xl p-6 hover:shadow-sm transition">
            ...
            <div className="grid grid-cols-2 gap-6">
              <div>
                <label className="font-medium text-gray-800">Título</label>
              ...
              <div>
                <label className="font-medium text-gray-800">Tipo</label>
              ...
              <div>
                <label className="font-medium text-gray-800">Prazo Final</label>
              ...
              <div>
                <label className="font-medium text-gray-800">Turma</label>
                ...
                >
                  ...
                  <option value="1A">1º EM A</option>

                  <option value="1B">1º EM B</option>

                  <option value="2A">2º EM A</option>

                  <option value="2B">2º EM B</option>

                  <option value="3A">3º EM A</option>
                </select>
              </div>

              <div>
                <label className="font-medium text-gray-800">Frequência</label>

                <select
                  value={dados.recorrencia}
                  onChange={(e) =>
                    atualizarCampo("recorrencia", e.target.value)
                  }
                  className="w-full mt-2 bg-[#F8F8FB] border border-gray-200 rounded-lg px-4 py-3 text-gray-500 placeholder:text-gray-400 outline-none focus:ring-2 focus:ring-[#0291F7] focus:border-[#0291F7]"
                >
                  <option value="uma-vez">Apenas uma vez</option>
            ...
            <div className="col-span-2 mt-6">
              <label className="font-medium text-gray-800">Destinatários</label>
      ...
      {showAlert && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl shadow-xl p-8 w-[420px]">
            ...
            <p className="text-gray-600 mb-8">{mensagemAlert}</p>
