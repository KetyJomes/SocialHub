...
export const AdminTurmas = () => {
  const confirmarAlteracaoStatus = (turma) => {
    ...
    Swal.fire({
      ...
      confirmButtonText: vaiArquivar ? "Sim, arquivar" : "Sim, desarquivar",

      cancelButtonText: "Cancelar",

      confirmButtonColor: vaiArquivar ? "#6B7280" : "#009C5D",
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire({
          title: vaiArquivar ? "Turma arquivada!" : "Turma desarquivada!",

          text: `${turma.nome} foi ${
            vaiArquivar ? "arquivada" : "desarquivada"
          } com sucesso.`,
  ...
  const fetchTurmas = async () => {
    const response = await api.get("/class/findAll");
    ...
    try {
      ...
      setTurmas([
        ...turmas,
        {
          ...
          turno: body.period,
          lider:
            professores.find((p) => p.id === Number(body.idPIC))?.nome || "",
          alunos: [],
          status: "Ativa",
        },
      ]);
  ...
  const turmasFiltradas = turmas.filter((turma) => {
    ...
    const statusSelecionado =
      filtroStatus === "Todas" || turma.status === filtroStatus;
      ...
      turmas.map((turma) =>
        turma.id === id
          ? {
              ...
              status: turma.status === "Ativa" ? "Arquivada" : "Ativa",
            }
          : turma
      )
    );
  ...
  const adicionarAluno = (aluno) => {
    ...
    setAlunosDisponiveis(
      alunosDisponiveis.filter((item) => item.id !== aluno.id)
    );

    setTurmaSelecionada({
      ...turmaSelecionada,

      alunos: [...turmaSelecionada.alunos, aluno],
    });
  };
  ...
  const removerAluno = (alunoId) => {
      ...
      turmas.map((turma) =>
        turma.id === turmaSelecionada.id
          ? {
              ...
              alunos: turma.alunos.filter((aluno) => aluno.id !== alunoId),
            }
          : turma
      )
    );
    ...
    setTurmaSelecionada({
      ...
      alunos: turmaSelecionada.alunos.filter((aluno) => aluno.id !== alunoId),
    });

    setAlunosDisponiveis([...alunosDisponiveis, alunoRemovido]);
  };
  ...
  const alunosDisponiveisFiltrados = alunosDisponiveis.filter(
    (aluno) =>
      ...
      aluno.email.toLowerCase().includes(pesquisaAluno.toLowerCase())
  );
        ...
        <div className="grid grid-cols-3 gap-6 mt-10">
          ...
          >
            ...
            <div>
              ...
              <p className="text-gray-500">Turmas</p>
            </div>
          </div>
          ...
          >
            ...
            <p className="text-gray-500">Alunos vinculados</p>
          </div>
            ...
            <thead>
              <tr
                className="
                        ...
                        "
              >
                <th className="text-center py-4">Turma</th>

                <th className="text-center">Turno</th>

                <th className="text-center">Líder</th>
            ...
            <tbody>
              {turmasFiltradas.map((turma) => (
                ...
                >
                  ...
                  <td className="text-center">{turma.lider}</td>

                  <td className="text-center">{turma.alunos.length}</td>
          ...
          >
            ...
            >
              <div>
                ...
                <p className="text-gray-500">Gerenciamento da turma</p>
              </div>
            ...
            >
              <div
                className="
                    ...
                    "
              >
                <p className="text-sm text-gray-500">Líder da turma</p>
                    ...
                    {turmaSelecionada.alunos.map((aluno) => (
                      <tr
                        ...
                        className="
                                                ...
                                                "
                      >
                        <td className="py-5">{aluno.nome}</td>
